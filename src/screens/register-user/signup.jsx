...
const signup = () => {
	...
	try {
		const result = await fetch("http://localhost:9000/api/v1/register", {
			method: "POST",
		...
		if (result) {
			...
			if (jsonResult.message === "user already exists please login") {
			...
			setError("error registering user"+ result.satus)
		}
	} catch (error) {
		setError(error)
	}
		...
		<div className="body">
			<div className="main">
				...
				<form onSubmit={RegisterUser}>
					...
					<div className="email">
						<p className="ptag">Phonenumber</p>
						<input
							className="inputinfo"
							type="tel"
							name="phonenumber"
					...
					<div className="reset">
						<Link to="/login" className="reset2">
							<p> Have an account?</p>
						</Link>
					</div>
				</form>
